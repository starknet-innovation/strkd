...
The vectors were generated by strkd (`cargo run --example conformance_vectors`)
and **verified against starknet.js** using exactly the call bramble makes,
`hash.calculateContractAddressFromHash(salt, class_hash, [public_key], 0)`.
Regenerating is not verifying: if a value changes, re-check it against an
independent implementation before committing, or the file only asserts that
strkd agrees with itself.

...
`strkd` (crate `strkd`, under `crates/wallet-cli/`) is a thin command-line
client for the running menu-bar app. It holds no keys — it discovers the app's loopback service via
...
Once a release is cut, end users install without a toolchain via the shell
installer, or by downloading a tarball from the release page:
...
documents exactly what to re-add.

## Releases

Two independent tracks, each triggered by its own tag shape, each publishing a
**draft** GitHub Release for a human to review and publish:

| Tag | Workflow | Ships |
| --- | --- | --- |
| `v0.1.0` | `release.yml` (generated by dist) | `strkd` CLI: shell installer + per-target tarballs + checksums |
| `desktop-v0.1.0` | `desktop-release.yml` | Menu-bar app with the prover bundled: `.dmg`, `.app.tar.gz`, `.deb`, `.rpm`, `.AppImage` |

```bash
git tag v0.1.0         && git push origin v0.1.0          # CLI
git tag desktop-v0.1.0 && git push origin desktop-v0.1.0  # desktop app
```
...
dist plan --tag v0.1.0     # dry run: exactly what a release would contain
dist build --artifacts=local --target aarch64-apple-darwin --tag v0.1.0
                           # build the real artifacts for one target, locally
```

While strkd is **alpha (test seeds only)**, prefer `cargo install` for internal
dogfooding and keep the public channels quiet.
